...
class CollapsiblePlatform():
    ...
    def _generate_field(self, env, case=1, sElasticStiffness=8, sDampingStiffness=1, texture=None):
        ...
        if case in [1,2]:
            ...
            env.pybullet_client.createSoftBodyAnchor(platform[-1], 4, -1, -1) # ground anchor on vertices 4,5,6,7. 
            env.pybullet_client.createSoftBodyAnchor(platform[-1], 5, -1, -1)
            env.pybullet_client.createSoftBodyAnchor(platform[-1], 6, -1, -1)
            env.pybullet_client.createSoftBodyAnchor(platform[-1], 7, -1, -1)
